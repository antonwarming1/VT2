...
def _predict_one_instance(screw_number, model_name="fnn", audio_mode=False, _print=True):
    ...
    for attempt in range(max_attempts):
        ...
        return ScrewResult(
            ...
            t_asset    = t_assets - t0,
            t_pipeline = t_pipe1 - t_pipe0,
            t_inference = t_inf1 - t_inf0,
        )
...
if __name__ == "__main__":
  ...
  for i in range(tests):
      ...
      times = pd.concat([times, pd.DataFrame([{
          "N": i + 1,
          "Label": result.label,
          "t_asset": time_result.t_asset,
          "t_pipeline": time_result.t_pipeline,
          "t_inference": time_result.t_inference,
          "t_total": time_result.t_asset + time_result.t_pipeline + time_result.t_inference,
          "ID": result.base_id,
      }])])


  distribution = times["Label"].value_counts(sort=True)
