...
# Collect all CSV/JSON files across subfolders
...
print("=" * 70)
# Find CSV files where time does not start at 0
for csv_path in ALL_CSV:
    df = pd.read_csv(csv_path)
    t0 = df["Time (ms)"].iloc[0]
    t_end = df["Time (ms)"].iloc[-1]
    if t0 != 0.0:
        print(f"\n  {csv_path.stem} ({csv_path.parent.name}):")
        print(f"    BEFORE: time = [{t0}, ..., {t_end}] ms")
        print(f"    AFTER:  time = [0.0, ..., {t_end - t0}] ms")
        print(f"    Fix: df['Time (ms)'] -= df['Time (ms)'].iloc[0]")
if all(pd.read_csv(f)["Time (ms)"].iloc[0] == 0.0 for f in ALL_CSV):
    print("\n  All files already start at 0 — no fix needed.")
...
print(f"\n  {label} time gaps (ms): min={diffs.min():.2f}, max={diffs.max():.2f}, "
      f"mean={diffs.mean():.2f}, std={diffs.std():.2f}")
print(f"  First 10 gaps: {diffs.head(10).values.tolist()}")
print(f"  Problem: gaps range from ~{diffs.min():.0f}ms to ~{diffs.max():.0f}ms (not uniform)")
...
print(f"\n  TCP_x is flat for the first {first_idx} rows ({df['Time (ms)'].iloc[first_idx]:.0f} ms)")
...
# Also show JSON idle
json_path = _first_json()
dfj = load_json_df(json_path)
...
# Find transitions
active_val = nset.max()
ramp_up_end = np.where(nset == active_val)[0][0] if active_val in nset else 0
ramp_down_start = len(nset) - 1 - np.where(nset[::-1] == active_val)[0][0] if active_val in nset else len(nset)
print(f"\n  {label} JSON — Nset (speed) profile:")
print(f"    Rows 0-{ramp_up_end}: RAMP-UP (speed goes 0 -> {active_val:.0f} rpm)")
print(f"    Rows {ramp_up_end}-{ramp_down_start}: ACTIVE SCREWING (constant {active_val:.0f} rpm)")
print(f"    Rows {ramp_down_start}-{len(nset)-1}: RAMP-DOWN (speed {active_val:.0f} -> 0 rpm)")
print(f"    Nset at boundaries: [{nset[0]}, {nset[ramp_up_end]}, ..., {nset[ramp_down_start]}, {nset[-1]}]")
print(f"    Fix:")
print(f"      ramp_mask = df['Nset'] < {active_val:.0f}")
print(f"      active = df[df['Nset'] == {active_val:.0f}]  # only the active screwing part")
...
with open(json_path) as f:
    raw = json.load(f)
# Find the Angle axis
for ax in raw["XML_Data"]["Wsk3Vectors"]["Y_AxesList"]["AxisData"]:
    if "angle" in ax["Header"]["Name"].lower():
        unit = ax["Header"]["Unit"]
        break
else:
    unit = "?"
...
json_lens = []
for f in ALL_CSV:
    csv_lens.append((f"{f.parent.name}/{f.stem}", len(pd.read_csv(f))))
for f in ALL_JSON:
    json_lens.append((f"{f.parent.name}/{f.stem}", len(load_json_df(f))))
